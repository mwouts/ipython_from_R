...
Hello
```
![<matplotlib.figure.Figure at 0xXXXXXXXXXXX>](figure/image-1.png)
```
Again
```
![<matplotlib.figure.Figure at 0xXXXXXXXXXXX>](figure/image-2.png)
```
Finally
```"""
    assert not err
...
Hello
\\end{verbatim}
\\includegraphics[width=\\linewidth]{figure/image-1}
\\begin{verbatim}
Again
\\end{verbatim}
\\includegraphics[width=\\linewidth]{figure/image-2}
\\begin{verbatim}
Finally
\\end{verbatim}"""
    assert not err

def test_matplotlib_and_animate_tex():
    config = parse_args(["""
import numpy as np
import matplotlib.pyplot as plt
...
plt.show()
print("Finally")
""", "--to", "latex", "--results", "markup", "--figshow", "animate"])
    r = execute(kc, config.code[0])
    err, result = format_result(r, config)
...
Finally
\\end{verbatim}
\\animategraphics[width=\\linewidth,controls,loop]{1}{figure/image-}{1}{2}"""
    assert not err
