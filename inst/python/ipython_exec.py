...
def parse_args(args):
    ...
    parser.add_argument('-s', '--figshow',
                        ...
                        dest='figshow',
                        default='asis',
                        choices=['asis', 'hold', 'hide', 'animate'],
...
def format_display(html, to='markdown'):
    if to == 'markdown':
        return '<!--html_preserve--><div class="display">\n' + html + '\n</div><!--/html_preserve-->'
    if to == 'asis':
        return html
    if to == "nohtml":
        return ''
...
def format_javascript(code, to='markdown'):
    if to == 'markdown':
        return '<!--/html_preserve--><script type="text/javascript">\n' + code + '\n</script><!--/html_preserve-->'
    if to == 'hide':
        return ''
...
def format_result(r, config):
    ...
    for msg_type, content in r:
        ...
        if msg_type == 'error':
            try:
                ...
                # error = '\n'.join(content['traceback']) # colored traceback :-(
        ...
        if msg_type == 'display_data':
            try:
                ...
                if image_data and config.figshow != 'hide':
                    filename = '%s-%d.%s' % (config.prefix, image_counter, image_ext)
                    image_counter += 1
                    ...
                    if config.figshow == 'animate':
                        if config.to != 'latex':
                            raise TypeError("Animations are only supported for LaTeX output")
                        images_in_hold = ['\\animategraphics[width=%s,controls,loop]{1}{%s-}{1}{%d}' % (config.outwidth, config.prefix, image_counter-1)]
                    elif config.figshow == 'hold':
    ...
    # don't append the images if there is an error
    # as we want the error message to be on the last line.
    if not error:
